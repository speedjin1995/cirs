...
if ($type == 'MULTI'){
	if(isset($_POST['id']) && $_POST['id']!=null && $_POST['id']!=""){
		...
		if ($status == "DELETE"){
			$del = 1;
			if ($stmt2 = $db->prepare("UPDATE other_validations SET deleted=?, update_datetime=? WHERE id IN ($ids)")) {
			...
			if ($stmt2 = $db->prepare("UPDATE other_validations SET status=?, reason_id=?, other_reason=?, update_datetime=? WHERE id IN ($ids)")) {
				$stmt2->bind_param('ssss', $del, $cancelReason, $otherReason, $updateDt);
				
				if($stmt2->execute()){
					// Cancel Stamping System Log
					foreach ($id_list as $id) {
						if ($insert_stmt3 = $db->prepare("INSERT INTO other_validation_log (action, user_id, item_id, cancel_id, remark) 
						VALUES (?, ?, ?, ?, ?)")){
							$action = "CANCELLED";
