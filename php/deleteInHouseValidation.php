...
if ($type == 'MULTI'){
	if(isset($_POST['id']) && $_POST['id']!=null && $_POST['id']!=""){
		...
		if ($status == "DELETE"){
			$del = 1;
			if ($stmt2 = $db->prepare("UPDATE inhouse_validations SET deleted=?, update_datetime=? WHERE id IN ($ids)")) {
			...
			if ($stmt2 = $db->prepare("UPDATE inhouse_validations SET status=?, reason_id=?, other_reason=?, update_datetime=? WHERE id IN ($ids)")) {
	...
	if(isset($_POST['id']) && $_POST['id']!=null && $_POST['id']!=""){
		...
		if($status == 'DELETE'){
			$del = 1;
			if ($stmt2 = $db->prepare("UPDATE inhouse_validations SET deleted=?, update_datetime=? WHERE id=?")) {
			...
			$del = "Cancelled";
			if ($stmt2 = $db->prepare("UPDATE inhouse_validations SET status=?, reason_id=?, other_reason=?, update_datetime=? WHERE id=?")) {
