        ...
        if (! $update_stmt->execute()) {
            ...
            if ($row = $result->fetch_assoc()) {
                ...
                if($format == 'EXPANDABLE') {
                    ...
                    $message['invoice_date'] = $row['invoice_date'] != null ? convertDatetimeToDate($row['invoice_date']) : '';
                    $message['po_attachment'] = $row['po_attachment'] ?? '';
                    $message['po_filepath'] = searchFilePathById($row['po_attachment'], $db) ?? '';
                    ...
                    $message['service_report_no'] = $row['service_report_no'] ?? '';
                    $message['service_report_attachment'] = $row['service_report_attachment'] ?? '';
                    $message['service_report_filepath'] = searchFilePathById($row['service_report_attachment'], $db) ?? '';
