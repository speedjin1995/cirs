...
if (!empty($data)) {
    foreach ($data as $rows) {
        ...
        $cid = searchDealerIdByName($CompanyName, $db);

        if($cid != null){
            $count = '1';

            if($misc_stmt = $db->prepare("SELECT COUNT(*) AS branchCodeCount FROM reseller_branches WHERE reseller_id=? AND deleted = '0'")){
            ...
            if($misc_stmt = $db->prepare("SELECT * FROM miscellaneous WHERE code='reseller' AND description=?")){
                ...
                if ($row = $result->fetch_assoc()){
                    ...
                    if($updmisc_stmt = $db->prepare("UPDATE miscellaneous SET value=? WHERE code='reseller' AND description=?")){
            ...
            if ($insert_stmt = $db->prepare("INSERT INTO dealer (customer_code, other_code, customer_name, customer_address, address2, address3, address4, map_url, customer_phone, customer_email, pic, pic_contact) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
                $insert_stmt->bind_param('ssssssssssss', $code, $OtherCodeAutoCountetc, $CompanyName, $Address1, $Address2, $Address3, $Address4, $MAPURL, $Phone, $Email, $PIC, $PICContact);
                $insert_stmt->execute();
