...
, $_POST['model'], $_POST['capacity'], $_POST['serial'])){
	...
	if(isset($_POST['id']) && $_POST['id'] != null && $_POST['id'] != ''){
		...
		, remarks=?, unit_price=?, cert_price=?, total_amount=?, sst=?, subtotal_amount=?, log=? WHERE id=?")){
			$data = json_encode($logs);
			$update_stmt->bind_param('ssssssssssssssssssssssssssssssssss', $customer, $address1, $address2, $address3, $brand, $machineType, $model, $capacity, $serial, 
			...
			$quotation, $quotationDate, $poNo, $poDate, $remark, $unitPrice, $certPrice, $totalPrice, $sst, $subtotalPrice, $data, $_POST['id']);
		...
		quotation_no, quotation_date, purchase_no, purchase_date, remarks, unit_price, cert_price, total_amount, sst, subtotal_amount, log) 
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")){
			$data = json_encode($logs);
			$insert_stmt->bind_param('ssssssssssssssssssssssssssssssssss', $customerType, $customer, $address1, $address2, $address3, $brand, $machineType, $model, $capacity, $serial, 
			$validator, $jenisAlat, $noDaftar, $pinKeselamatan, $siriKeselamatan, $includeCert, $borangD, $invoice, $cashBill, $stampDate, $dueDate, $uid, $pic, 
			$quotation, $quotationDate, $poNo, $poDate, $remark, $unitPrice, $certPrice, $totalPrice, $sst, $subtotalPrice, $data);
