...
$tempDir = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'temp_ujian';
if (!is_dir($tempDir)) {
...
if(isset($_GET['userID'], $_GET['actualPrintDate'])){
    $id = $_GET['userID'];
        ...
        foreach ($rows as $res) {
            ...
            if($file == 'ATK' && $validator == 'METROLOGY'){
                ...
                if(!empty($customerRow)){
                    ...
                    $hqAddress5 = $customerRow['address5'];
                }
                ...
                for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                    ...
                    if ($pageNo == 1) {
                        ...
                        $pdf->SetXY(24.599, 40); // Adjust these coordinates for each field
                        ...
                        $pdf->SetXY(24.599, 47); // Adjust these coordinates for each field
                        $pdf->Write(0, $hqAddress1 . ' ' . $hqAddress2);

                        $pdf->SetXY(24.599, 53.5); // Adjust these coordinates for each field
                        $pdf->Write(0, $hqAddress3.' '.$hqAddress4);

                        $pdf->SetXY(24.599, 60); // Adjust these coordinates for each field
                        $pdf->Write(0, $hqAddress5);

                        $pdf->SetXY(24.599, 72.5); // Adjust these coordinates for each field
                        $pdf->Write(0, $address1.' '.$address2);

                        $pdf->SetXY(24.599, 79); // Adjust these coordinates for each field
                        $pdf->Write(0, $address3.' '.$address4);

                        $pdf->SetXY(24.599, 85); // Adjust these coordinates for each field
                        $pdf->Write(0, $address5);

                        $pdf->SetXY(75, 104); // Adjust for {Company_Name}
                        $pdf->Write(0, $compname);

                        $pdf->SetXY(50, 109.5); // Adjust for {No_Lesen}
                        $pdf->Write(0, $compcert);

                        $pdf->SetXY(155, 109.5); // Adjust for {Tarikh_Tamat_Lesen}
                        $pdf->Write(0, $compexp);

                        $pdf->SetXY(72, 115); // Adjust for {Nama_Wakil_Pembaik}
                        $pdf->Write(0, searchStaffNameById($res['assignTo'], $db));

                        $pdf->SetXY(135, 115); // Adjust for {No_KP}
                        $pdf->Write(0, searchStaffICById($res['assignTo'], $db));

                        $pdf->SetXY(68, 129); // Adjust for {Penentusahan_Baru}
                        $pdf->Write(0, $res['penentusan_baru']);

                        $pdf->SetXY(158, 129); // Adjust for {Penentusahan_Semula}
                        $pdf->Write(0, $res['penentusan_semula']);

                        if($res['kelulusan_mspk'] == 'YES'){
                            $pdf->Image($tickImage, 47, 148, 6); // Adjust for {NoKelulusan_MSPK}
                        }
                        else{
                            $pdf->Image($tickImage, 47, 153, 6); // Adjust for {NoKelulusan_MSPK}
                        }
                        
                        $pdf->SetXY(110, 151); // Adjust for {NoKelulusan_MSPK}
                        $pdf->Write(0, ": ".$res['no_kelulusan']);

                        $pdf->SetXY(71, 174); // Adjust for {Pembuat_Negara_Asal}
                        $pdf->Write(0, searchCountryById($res['make_in'], $db));

                        $pdf->SetXY(133, 174); // Adjust for {Jenama}
                        $pdf->Write(0, searchBrandNameById($res['brand'], $db));

                        $pdf->SetXY(42, 178.5); // Adjust for {Model#1}
                        $pdf->Write(0, searchModelNameById($res['model'], $db));

                        $pdf->SetXY(135, 178.5); // Adjust for {No_Siri}
                        $pdf->Write(0, $res['serial_no']);

                        $pdf->SetXY(75, 193); // Adjust for {Pembuat_Negara_Asal_2}
                        $pdf->Write(0, searchCountryById($res['platform_country'], $db));

                        $pdf->SetXY(160, 193); // Adjust for {Jenis_Steel_Concrete}
                        $pdf->Write(0, $res['platform_type']);

                        $pdf->SetFont('', '', 8); // B stands for Bold
                        $pdf->SetXY(75, 199.5); // Adjust for {size}
                        $pdf->Write(0, searchSizeNameById($res['size'], $db));

                        $pdf->SetFont('', '', 10); // B stands for Bold
                        $pdf->SetXY(62, 205); // Adjust for {size}
                        $pdf->Write(0, $res['other_info']);

                        if($res['jenis_pelantar'] == 'Pit'){
                            $pdf->Ellipse(145, 200, 4, 3, 'D', [200, 255, 200]);
                        }
                        else{
                            $pdf->Ellipse(156, 200, 7, 3, 'D', [200, 255, 200]);
                        }

                        $pdf->SetXY(72, 218.5); // Adjust for {Pembuat_Negara_Asal_3}
                        $pdf->Write(0, searchCountryById($res['load_cell_country'], $db));

                        $pdf->SetXY(151, 218.5); // Adjust for {Bilangan_Load_Cell}
                        $pdf->Write(0, $res['load_cell_no']);

                        $count = 0;
                        for($i=0; $i<count($loadcells); $i++){
                            $pdf->SetXY(24, 233 + $count); // Adjust for {Bilangan_Load_Cell}
                            $pdf->Write(0, $loadcells[$i]['no']);

                            $pdf->SetXY(31, 233 + $count); // Adjust for {Bilangan_Load_Cell}
                            $pdf->Write(0, $loadcells[$i]['loadCellBrand']);

                            $pdf->SetXY(72, 233 + $count); // Adjust for {Bilangan_Load_Cell}
                            $pdf->Write(0, $loadcells[$i]['loadCellModel']);

                            $pdf->SetXY(111, 233 + $count); // Adjust for {Bilangan_Load_Cell}
                            $pdf->Write(0, $loadcells[$i]['loadCellCapacity']);

                            $pdf->SetXY(130, 233 + $count); // Adjust for {Bilangan_Load_Cell}
                ...
                for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                    ...
                    if ($pageNo == 1) {
                        // Fill in the fields at the appropriate positions
                        $pdf->SetFont('', 'B', 10); // B stands for Bold
                        $pdf->SetXY(20, 68); // Adjust these coordinates for each field
                        $pdf->Write(0, searchCustNameById($res['customers'], $db)); // {Address1}
                        $pdf->SetFont('', '', 10); // B stands for Bold

                        $pdf->SetXY(20, 75); // Adjust for {Address2}
                        $pdf->Write(0, $hqAddress1.' '.$hqAddress2);

                        $pdf->SetXY(20, 82); // Adjust for {Address2}
                        $pdf->Write(0, $hqAddress3.' '.$hqAddress4);

                        $pdf->SetXY(20, 90); // Adjust for {Address2}
                        $pdf->Write(0, $hqAddress5);

                        $pdf->SetXY(20, 105); // Adjust for {Stamping_Address1}
                        $pdf->Write(0, $address1.' '.$address2);

                        $pdf->SetXY(20, 112); // Adjust for {Stamping_Address2}
                        $pdf->Write(0, $address3.' '.$address4);

                        $pdf->SetXY(20, 119); // Adjust for {Stamping_Address2}
                        $pdf->Write(0, $address5);

                        $pdf->SetXY(65, 139); // Adjust for {Company_Name}
                        $pdf->Write(0, $compname);

                        $pdf->SetXY(45, 144); // Adjust for {No_Lesen}
                        $pdf->Write(0, $compcert);

                        $pdf->SetXY(60, 149.5); // Adjust for {Tarikh_Tamat_Lesen}
                        $pdf->Write(0, $compexp);

                        $pdf->SetXY(58, 154.5); // Adjust for {Nama_Wakil_Pembaik}
                        $pdf->Write(0, searchStaffNameById($res['assignTo'], $db));

                        $pdf->SetXY(38, 160); // Adjust for {No_KP}
                        $pdf->Write(0, searchStaffICById($res['assignTo'], $db));

                        $pdf->SetXY(60, 180); // Adjust for {Penentusahan_Baru}
                        $pdf->Write(0, $res['penentusan_baru']);

                        $pdf->SetXY(64, 186); // Adjust for {Penentusahan_Semula}
                        $pdf->Write(0, $res['penentusan_semula']);

                        if($res['kelulusan_mspk'] == 'YES'){
                            $pdf->Image($tickImage, 35, 214, 6);
                        }
                        else{
                            $pdf->Image($tickImage, 35, 220, 6);
                        }

                        $pdf->SetXY(95, 217.5); // Adjust for {NoKelulusan_MSPK}
                        $pdf->Write(0, $res['no_kelulusan']);

                        $pdf->SetXY(66, 244); // Adjust for {Pembuat_Negara_Asal}
                        $pdf->Write(0, searchCountryById($res['make_in'], $db));

                        $pdf->SetXY(40, 249); // Adjust for {Jenama}
                        $pdf->Write(0, searchBrandNameById($res['brand'], $db));

                        $pdf->SetXY(36, 254.5); // Adjust for {Model#1}
                        $pdf->Write(0, searchModelNameById($res['model'], $db));

                        $pdf->SetXY(40, 259.5); // Adjust for {No_Siri}
                        $pdf->Write(0, $res['serial_no']);
                    }
                    else if ($pageNo == 2){
                ...
                for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                    ...
                    if ($pageNo == 1) {
                        ...
                        $pdf->SetXY(20.648, 124); // Adjust for {3. Alamat Pemilik Address 3}
                        $pdf->Write(0, $address3);
                        
                        $pdf->SetXY(20.648, 130); // Adjust for {3. Alamat Pemilik Address 4}
                        $pdf->Write(0, $address4);

                        $pdf->SetXY(20.648, 135); // Adjust for {3. Alamat Pemilik Address 5}
                        $pdf->Write(0, $address5);
                ...
                for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                    ...
                    if ($pageNo == 1) {
                        ...
                        $pdf->SetXY(15.288, 110.305-2); // Adjust for {3. Alamat Pemilik Address 3 & 4}
                ...
                for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                    ...
                    if ($pageNo == 1) {
                        ...
                        $pdf->SetFillColor(255, 255, 255);  // cover up unneccesary text
                        $pdf->Rect(14.648, 85, 70, 20, 'F');

                        $pdf->SetFont('Arial', 'B', 8);
                        $pdf->SetXY(14.648, 87.063); // Adjust for Customer Name
                        $pdf->Write(0, searchCustNameById($res['customers'], $db));
                        $pdf->SetFont('Arial', '', 8);

                        $pdf->SetXY(14.648, 91.133); // Adjust for {3. Alamat Pemilik Address 1}
                        $pdf->Write(0, $address1);

                        $pdf->SetXY(14.648, 96.188); // Adjust for {3. Alamat Pemilik Address 2}
                        $pdf->Write(0, $address2);
                        
                        $pdf->SetXY(14.648, 101.258); // Adjust for {3. Alamat Pemilik Address 3 & 4}
                        $pdf->Write(0, $address3);

                        $pdf->SetXY(14.648, 106.258); // Adjust for {3. Alamat Pemilik Address 3 & 4}
                        $pdf->Write(0, $address4 . ' ' . $address5);
                        $pdf->SetFont('Arial', '', 10);
                ...
                for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                    ...
                    if ($pageNo == 1) {
                        ...
                        $pdf->SetXY(15.666, 112.855-2); // Adjust for {3. Alamat Pemilik Address 4}
                        $pdf->Write(0, $address5);
            ...
            else if(str_contains($file, 'ATP') && $validator == 'METROLOGY'){
                $fillFile = 'forms/Metrology/ATP_FORM.pdf';
                $pageCount = $pdf->setSourceFile($fillFile);
            ...
            else if(str_contains($file, 'ATP') && $validator == 'DE METROLOGY'){
                $fillFile = 'forms/DE_Metrology/DMSB_ATP.pdf';
                $pageCount = $pdf->setSourceFile($fillFile);
                ...
                for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                    ...
                    if ($pageNo == 1) {
                        ...
                        $pdf->SetXY(18.648, 122.258); // Adjust for {3. Alamat Pemilik Address 4}
                        $pdf->Write(0, $address4);
                ...
                for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                    ...
                    if ($pageNo == 1) {
                        ...
                        $pdf->SetXY(8.648, 121.258-2); // Adjust for {3. Alamat Pemilik Address 5}
                        $pdf->Write(0, $address5);
                ...
                for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                    ...
                    if ($pageNo == 1) {
                        ...
                        $pdf->SetXY(15.666, 71.188-2); // Adjust for {3. Alamat Pemilik Address 3 & 4 & 5}
                ...
                $fillFile = 'forms/Metrology/SIC_FORM.pdf';
                $pageCount = $pdf->setSourceFile($fillFile);
                ...
                for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                    ...
                    if ($pageNo == 1) {
                        // Fill in the fields at the appropriate positions
                        $pdf->SetFont('Arial', 'B', 10);
                        $pdf->SetXY(80.283, 51.668-2); // Adjust for Customer Name
                        $pdf->Write(0, searchCustNameById($res['customers'], $db));
                        $pdf->SetFont('Arial', '', 10);

                        $pdf->SetXY(15.608, 58.812-2); // Adjust for {3. Alamat Pemilik Address 2}
                        $pdf->Write(0, $address1 . ' ' .$address2);
                        ...
                        $pdf->SetXY(35.469, 98.5); // Adjust for {No_Lesen}
                        $pdf->Write(0, $compcert);

                        $pdf->SetXY(125.122, 90.448-2); // Adjust for {Nama_Wakil_Pembaik}
                        $pdf->Write(0, searchStaffNameById($res['assignTo'], $db));

                        $pdf->SetXY(128.120, 98.5); // Adjust for {No_KP}
                        $pdf->Write(0, searchStaffICById($res['assignTo'], $db));

                        $pdf->SetXY(45.840, 110.045-1.5); // {Borang_e}
                        $pdf->Write(0, $res['borang_e']); 

                        $pdf->Image($tickImage, 174, 115.5, 6); // {Kegunaan_alat}

                        $pdf->SetXY(43.679, 130.241-2); // {model}
                        $pdf->Write(0, searchModelNameById($res['model'], $db));

                        $pdf->SetXY(85.840, 140.045-2); // {nilai_jangkaan_maksimum}
                        $pdf->Write(0, $res['nilai_jangkaan_maksimum']); 

                        # Adjust for {bahan_pembuat}
                        if ($res['bahan_pembuat'] == 'PANCALOGAM'){
                            $pdf->Image($tickImage, 65, 154.637, 6);
                        }elseif ($res['bahan_pembuat'] == 'LOGAM BERENAMEL'){
                            $pdf->Image($tickImage, 65, 164.637, 6);
                        }elseif ($res['bahan_pembuat'] == 'BESI BERSADUR'){
                            $pdf->Image($tickImage, 65, 174.637, 6);
                        }elseif ($res['bahan_pembuat'] == 'KACA'){
                            $pdf->Image($tickImage, 174, 154.637, 6);
                        }elseif ($res['bahan_pembuat'] == 'TEMBIKAR'){
                            $pdf->Image($tickImage, 174, 164.637, 6);
                        }elseif ($res['bahan_pembuat'] == 'KELULI'){
                            $pdf->Image($tickImage, 174, 174.637, 6);
                        }elseif ($res['bahan_pembuat'] == 'OTHER'){
                            $pdf->Image($tickImage, 65, 184, 6);
                            $pdf->SetXY(22.837, 192);
                            $pdf->Write(0, $res['bahan_pembuat_other']); 
                        }

                        if (isset($companySignature) && $companySignature!=null && $companySignature!=""){
                            $pdf->Image($companySignature, 125.664, 213.083, 41);  // Adjust for company signature
                        }

                        if ($res['stamping_type'] == 'RENEWAL'){
                            $pdf->SetXY(39.404, 211.388); // Adjust for {tarikh}
                            if (!empty($res['last_year_stamping_date'])){
                                $pdf->Write(0, date("d/m/Y", strtotime($res['last_year_stamping_date']))); 
                            } 

                            $pdf->SetXY(42.059, 224.213-2.5); // Adjust for {Cawangan}
                            $pdf->Write(0, searchStateNameById($res['cawangan'], $db));
    
                            $pdf->SetXY(56.846, 234.162-3); // Adjust for {no_penentusahan}
                            $pdf->Write(0, $res['no_daftar_lama']);
                        }
                    }
                }
                ...
                $fillFile = 'forms/Metrology/LITENG_FORM.pdf';
                $pageCount = $pdf->setSourceFile($fillFile);
                ...
                for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                    ...
                    if ($pageNo == 1) {
                        // Fill in the fields at the appropriate positions
                        $pdf->SetXY(74, 53.382); // Adjust for Customer Name
                        $pdf->Write(0, searchCustNameById($res['customers'], $db). ', ' . $address1 . ' ' . $address2);
                        ...
                        // Adjust for {Keadaan Alat}
                        if ($res['stamping_type'] == 'NEW'){
                            $pdf->Image($tickImage, 108, 67, 6); 
                        }elseif ($res['stamping_type'] == 'RENEWAL'){
                            $pdf->Image($tickImage, 135, 67, 6);
                        }

                        if ($res['stamping_type'] == 'RENEWAL'){
                            $pdf->SetXY(60, 85); // Adjust for {tarikh}
                            if (!empty($res['last_year_stamping_date'])){
                                $pdf->Write(0, date("d/m/Y", strtotime($res['last_year_stamping_date']))); 
                            } 

                            $pdf->SetXY(60, 90); // Adjust for {Cawangan}
                            $pdf->Write(0, searchStateNameById($res['cawangan'], $db));
                ...
                for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                    ...
                    if ($pageNo == 1) {
                        // Fill in the fields at the appropriate positions
                        $pdf->SetXY(70, 61); // Adjust for Customer Name
                        $pdf->Write(0, searchCustNameById($res['customers'], $db));

                        $pdf->SetFont('Arial', '', 10);
                        $pdf->SetXY(15, 67); // Adjust for Customer Address
                        $pdf->Write(0, $address2 . ' ' . $address3);
                        ...
                        $pdf->SetXY(40, 106); // Adjust for {Capacity}
                        $pdf->Write(0, $capacityValue);

                        // Adjust for {Keadaan Alat}
                        if ($res['stamping_type'] == 'NEW'){
                            $pdf->Image($tickImage, 39, 94, 6); 
                        }elseif ($res['stamping_type'] == 'RENEWAL'){
                            $pdf->Image($tickImage, 65, 93, 6);
                        }

                        if ($res['stamping_type'] == 'RENEWAL'){
                            $pdf->SetXY(150, 88.5); // Adjust for {tarikh}
                            if (!empty($res['last_year_stamping_date'])){
                                $pdf->Write(0, date("d/m/Y", strtotime($res['last_year_stamping_date']))); 
                            } 

                            $pdf->SetXY(157, 97); // Adjust for {Cawangan}
                            $pdf->Write(0, searchStateNameById($res['cawangan'], $db));
                ...
                $btuBoxInfo = json_decode($res['btu_box_info'], true);
                    ...
                    foreach ($btuBoxInfo as $btuBox){
                        ...
                        for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                            ...
                            if ($pageNo == 1) {
                                ...
                                # Adjust for {Alat Type}
                                if ($btuBox['batuUjian'] == 'BESI_TUANGAN'){
                                    $pdf->Image($tickImage, 148.238+22, 108.866-6, 6);
                                }elseif ($btuBox['batuUjian'] == 'TEMBAGA'){
                                    $pdf->Image($tickImage, 148.238+22, 121.516-6, 6);
                                }elseif ($btuBox['batuUjian'] == 'NIKARAT'){
                                    $pdf->Image($tickImage, 148.238+22, 133.995-6, 6);
                                }elseif ($btuBox['batuUjian'] == 'OTHER'){
                                    $pdf->SetXY(128.526, 146.034-2); // Adjust for {Nilai Jangkaan}
                                    $pdf->Write(0, $btuBox['batuUjianLain']);                                 
                                    $pdf->Image($tickImage, 148.238+22, 146.034-4, 6);
                                }
            ...
            if ($file == 'BTU - (BOX)'){
                ...
                $btuMergedPdf = new PDFWithEllipse();

                foreach ($btuBoxFilePath as $file) {
                    $pageCount = $btuMergedPdf->setSourceFile($file);

                    for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
                        $templateId = $btuMergedPdf->importPage($pageNo);
                ...
                $btuMergedPdf->Output('F', $outputPath);
