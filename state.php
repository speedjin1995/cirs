				...
				<div class="card">
					<div class="card-header">
                        <div class="row">
                            <div class="col-8"></div>
                            <div class="col-2">
                                <button type="button" class="btn btn-block btn-sm bg-gradient-danger" id="multiDeactivate" data-bs-toggle="tooltip" title="Delete State"><i class="fa-solid fa-ban"></i> Delete State</button>
                            </div>
                            <div class="col-2">
                                <button type="button" class="btn btn-block btn-sm bg-gradient-warning" id="addUnits"><i class="fa-solid fa-circle-plus"></i> Add State</button>
                            </div>
                        </div>
                    </div>
					<div class="card-body">
        ...
        <form role="form" id="unitForm">
            ...
            <div class="modal-body">
                <div class="card-body">
    				...
    				<div class="form-group">
    					<label for="units">State *</label>
    					<input type="text" class="form-control" name="units" id="units" placeholder="Enter State" required>
    				</div>
    			</div>
            </div>
...
$(function () {
    $('#selectAllCheckbox').on('change', function() {
        var checkboxes = $('#unitTable tbody input[type="checkbox"]');
    ...
    $.validator.setDefaults({
        submitHandler: function () {
            $('#spinnerLoading').show();
            $.post('php/state.php', $('#unitForm').serialize(), function(data){
    ...
    $('#multiDeactivate').on('click', function () {
        ...
        $("#unitTable tbody input[type='checkbox']").each(function () {
            if (this.checked) {
        ...
        if (selectedIds.length > 0) {
            if (confirm('Are you sure you want to cancel these items?')) {
                $.post('php/deleteState.php', {userID: selectedIds, type: 'MULTI'}, function(data){
                    var obj = JSON.parse(data);
...
function edit(id){
    $('#spinnerLoading').show();
    $.post('php/getState.php', {userID: id}, function(data){
        var obj = JSON.parse(data);
        
        if(obj.status === 'success'){
            $('#unitModal').find('#id').val(obj.message.id);
            $('#unitModal').find('#units').val(obj.message.state);
            $('#unitModal').modal('show');
...
function deactivate(id){
    if (confirm('Are you sure you want to cancel this item?')) {
        $('#spinnerLoading').show();
        $.post('php/deleteState.php', {userID: id}, function(data){
            var obj = JSON.parse(data);
