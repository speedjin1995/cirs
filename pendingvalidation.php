      ...
      <div class="col-lg-12">
        <div class="card card-primary">
          <div class="card-header">
            <div class="row">
              <div class="col-8"><p class="mb-0" style="font-size: 110%">Other Validation Record Pending / Expired Status</p></div>
              <div class="col-2">
          ...
          <!-- <div class="card card-primary">
            <div class="card-body">
              ...
              <div class="row">
                ...
                <div class="col-2">
                  <button type="button" class="btn btn-primary add-load-cell" id="add-calibration-cell" hidden>Add Calibration</button>
                </div>
              </div>
  ...
  <div class="modal-dialog modal-xl" style="max-width: 50%;">
    <div class="modal-content">
      ...
      <form role="form" id="duplicateForm">
        <div class="modal-header bg-gray-dark color-palette">
          <h4 class="modal-title">Duplicate Validation</h4>
          <button type="button" class="close bg-gray-dark color-palette" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
...
<script type="text/html" id="loadCalibrationDetails">
  <tr class="details">
    ...
    <td>
      <input type="text" id="autoFormNo" name="autoFormNo" placeholder="Certificate Number" style="width: 100%;">
    </td>
    <td>
      <div class="row">
        <div class="col-10">
...
$(function () {
  ...
  var table = $("#weightTable").DataTable({
    ...
    'columns': [
      ...
      //     // System Log
      //     buttons += '<div class="col-6"><button title="Log" type="button" id="log'+data+'" onclick="log('+data+')" class="btn btn-info btn-sm"><i class="fa fa-list" aria-hidden="true"></i></button></div>';
  ...
  $.validator.setDefaults({
    submitHandler: function () {
      if($('#cancelModal').hasClass('show')){
        ...
        $.post('php/duplicateValidation.php', $('#duplicateForm').serialize(), function(data){
          var obj = JSON.parse(data); 
          if(obj.status === 'success'){
  ...
  $(document).on('click', '#add-calibration-cell', function() {
      ...
      $("#loadCalibrationTable").find('#autoFormNo:last').attr('name', 'autoFormNo[' + loadCalibrationCount + ']').attr("id", "autoFormNo" + loadCalibrationCount);
...
function newEntry(){
  ...
  $('#extendModal').find('#validationDate').val(formatDate3(date));
  $('#extendModal').find('#lastCalibrationDate').val('');
  $('#extendModal').find('#expiredCalibrationDate').val('');
  $('#extendModal').find('#autoFormNo').val('');
  ...
  // $('#add-calibration-cell').trigger('click');
