				...
				<div class="card">
					<div class="card-header">
                        <div class="row">
                            <div class="col-8"></div>
                            <div class="col-2">
                                <button type="button" class="btn btn-block btn-sm bg-gradient-danger" id="multiDeactivate" data-bs-toggle="tooltip" title="Delete Country"><i class="fa-solid fa-ban"></i> Delete Country</button>
                            </div>
                            <div class="col-2">
                                <button type="button" class="btn btn-block btn-sm bg-gradient-warning" id="addUnits"><i class="fa-solid fa-circle-plus"></i> Add Country</button>
                            </div>
                        </div>
                    </div>
					<div class="card-body">
        ...
        <form role="form" id="unitForm">
            ...
            <div class="modal-body">
                <div class="card-body">
    				...
    				<div class="form-group">
    					<label for="units">Country *</label>
    					<input type="text" class="form-control" name="units" id="units" placeholder="Enter Country" required>
    				</div>
    			</div>
            </div>
...
$(function () {
    ...
    $.validator.setDefaults({
        submitHandler: function () {
            $('#spinnerLoading').show();
            $.post('php/country.php', $('#unitForm').serialize(), function(data){
    ...
    $('#multiDeactivate').on('click', function () {
        ...
        if (selectedIds.length > 0) {
            if (confirm('Are you sure you want to cancel these items?')) {
                $.post('php/deleteCountry.php', {userID: selectedIds, type: 'MULTI'}, function(data){
                    var obj = JSON.parse(data);
...
function edit(id){
    $('#spinnerLoading').show();
    $.post('php/getCountry.php', {userID: id}, function(data){
        var obj = JSON.parse(data);
...
function deactivate(id){
    if (confirm('Are you sure you want to cancel this item?')) {
        $('#spinnerLoading').show();
        $.post('php/deleteCountry.php', {userID: id}, function(data){
            var obj = JSON.parse(data);
